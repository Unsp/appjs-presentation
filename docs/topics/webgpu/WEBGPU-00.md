# WEBGPU-00 — Research: WebGPU on RN (scope lock)

**Status:** done  
**Closed:** 2026-06-16 (owner confirmed scope in chat)  
**Feature:** [docs/topics/webgpu/README.md](README.md)  
**Outline:** [docs/outline/README.md](../../outline/README.md) — talk slot 4, title **WebGPU в React Native**  
**Blocks:** [WEBGPU-01](WEBGPU-01.md) (Expo demo), [WEBGPU-02](WEBGPU-02.md) (slides)  
**Related:** [KEYFRAMER-02](../keyframer/KEYFRAMER-02.md) (cancelled — Expo bootstrap moves here)

## Owner scope lock (final — 2026-06-16)

Owner confirmed — stop here; WEBGPU-01 may start.

### Segment narrative

- **Title:** WebGPU в React Native (TypeGPU — отдельный слайд, не в заголовке)
- **Thesis:** WebGPU теперь в RN; популярные либы переезжают на WebGPU; интересные DX-слои вроде TypeGPU
- **Slides (WEBGPU-02):** Block A WebGPU → Block B TypeGPU (+ threading) → Block C Skia Graphite `@next` (slides only) → live demo
- **A2 / A3:** отдельные слайды пока; merge TBD в WEBGPU-02

### Live demo (WEBGPU-01)

Single **stress-test** screen — one screen proves **TypeGPU top-3 business pitch**:

| Pitch use case                       | In demo                                                                        |
| ------------------------------------ | ------------------------------------------------------------------------------ |
| Custom live viz (mini chart / strip) | GPU lane — `'use gpu'` + `time` uniform; optional `FunctionVisualizer` stretch |
| GPU under busy JS                    | **Core** — `runOnUI` + `installWebGPU` vs Reanimated baseline; «Нагрузить JS»  |
| Branded canvas (shimmer/glow)        | Same GPU shader — shimmer; no confetti                                         |

**Out of demo:** Redraw, confetti, Skia Graphite, blur/frosted glass live, separate tabs.

### Business messaging (locked)

- **Blur / glass UI** — narrative context only (stable Skia today); **not** WebGPU novelty pitch
- **WebGPU value:** programmable GPU in TS, perf under JS load, ecosystem convergence
- **Devices:** iOS primary; Android best-effort
- **Expo SDK:** 56

Full detail: README — [Demo scope decision](README.md#demo-scope-decision), [Slide beat plan](README.md#slide-beat-plan-discussion--typegpu-02-only), [TypeGPU business use cases](README.md#typegpu-business-use-cases-demo-mapping).

## Goal

**Research and scope lock only** — no `src/` bootstrap; slide edits out of scope for 00 (earlier inline stub edits left per owner).

Produce owner-reviewable feasibility report in `docs/topics/webgpu/README.md`. **Owner confirmed scope 2026-06-16.**
...
Output lives in `docs/topics/webgpu/README.md` (Research findings + Demo scope decision sections). Owner confirms scope in chat before WEBGPU-01 starts.
...
| 4   | Redraw decision     | Explicit **include** or **cut** with rationale — no “maybe” left for WEBGPU-01                   |
...
| 9   | KEYFRAMER handoff   | Note whether keyframer export screen fits WEBGPU-01 or stays post-talk (see KEYFRAMER-02)        |
| 10  | No code             | No `src/`, no `slides/pages/` edits in this task                                                  |
| 11  | Owner gate          | Agent posts report in Russian; owner replies with approved scope before WEBGPU-01                |
...
- `docs/topics/webgpu/README.md` — primary deliverable
