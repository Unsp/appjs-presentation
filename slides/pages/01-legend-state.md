...
<!--
Например если нам нужно поставить запрос на паузу - мы меняет стейт флага, что вызывает перерисовку, а также перезапрос данных, что вызвет еще 1 ререндер
-->
...
<!--
А теперь поговорим о коллбеках. Наверняка многие из вас заметили здесь ошибку. 

Массив зависимостей пустой, что приведет к тому, что мы будет получать устаревшее значение в логе
-->
...
<!--
Ок, мы это исправили. Однако что присходит теперь? При каждом изменении value у нас будет пересоздаваться функция, что в свою очередь вызовет перерисовку большого компонента. И это скрытая проблема, которую так просто не обнаружишь
-->
...
<!--
Решение этих проблем возможно благодаря observable объектам, так же известными как...
-->
...
<div class="relative w-full h-full p-3 box-border flex flex-1 flex-row gap-4">
  ...
  <div class="flex flex-1 items-start flex-col">
    ...
    <ul class="!text-white text-sm space-y-2 list-disc pl-4 m-0 flex flex-1 flex-col">
      <li><strong>Не подписывается на состояние</strong></li>
      <li><strong>Подписка происходит отдельно</strong></li>
      <li><strong>Разделяет Ownership и Subscription</strong></li>
      <li><strong>Поддерживает стабильные ссылки (Stable References)</strong></li>
      <li><strong>Позволяет переносить подписки вниз по дереву</strong></li>
      <li><strong>Минимизирует ререндеры</strong></li>
    </ul>
  </div>
</div>
...
<!--
Давайте вспомним что происходит при измении глобального стейта в классическом подходе
-->
...
<!--
При использовании observable объектов - сайд эффекты больше не зависят от рендера при изменении стейта
-->
